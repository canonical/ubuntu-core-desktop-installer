...
void miriway::WindowManagerPolicy::handle_modify_window(WindowInfo& window_info, WindowSpecification const& modifications)
{
    ...
    if (mods.size().is_set())
    {
        auto const application_zone = tools.active_application_zone().extents();

        auto centered = application_zone.top_left
                        + 0.5*(as_displacement(application_zone.size) - as_displacement(mods.size().value()))
                        - (as_delta(application_zone.size.height) - as_delta(mods.size().value().height)) / 6;

        mods.top_left() = centered;
    }
